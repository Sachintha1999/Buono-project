// ════════════════════════════════════════════════════════════
// 🛒 PURCHASING DATABASE - OPTIMIZED JAVASCRIPT
...
const Perf = {
    ...
    start(label) { this._marks[label] = performance.now(); },
        ...
        console.log(`⚡ [PURCHASING] ${label}: ${t.toFixed(1)}ms`);
        return t;
    }
};

// ─── Cache Keys ───
const USER_CACHE_KEY      = 'buono_purch_user_v1';
const SUPPLIERS_CACHE_KEY = 'buono_purch_suppliers_v1';
const INVENTORY_CACHE_KEY = 'buono_purch_inventory_v1';
...
function cacheGet(key, maxAge = CACHE_5MIN) {
    try {
...
function revealPage() {
    ...
    const overlay = document.getElementById('purchLoadingOverlay');
    const content = document.getElementById('purchMainContent');
    ...
    console.log('✅ [PURCHASING] Page revealed! Console timings 👆');
}
...
async function initializeApp() {
    ...
    const cachedUser = cacheGet(USER_CACHE_KEY);
    if (cachedUser) {
        console.log('✅ [PURCHASING] User cache hit!');
        Perf.end('User Auth');
    ...
    // ⚡ Parallel load: Inventory + Suppliers
    Perf.start('Parallel Data Load');
    await Promise.all([
        loadInventoryItems(),
        loadSuppliersData()
    ]);
    Perf.end('Parallel Data Load');
...
async function refreshUserBackground(userId) {
    try {
        ...
        if (userDoc.exists) {
            ...
            console.log('🔄 [PURCHASING] User cache refreshed background');
        }
...
function initializeReturnsUI() {
    document.querySelectorAll('.return-sub-section').forEach((sec, i) => {
...
function showSection(name, btnEl) {
    ...
    if (name === 'rejected')  {
        ...
        updateBadges(); updateAlertBanner();
    }
    if (name === 'credit')    renderCreditTab();
    if (name === 'suppliers') { renderSuppliersList(allSuppliers); updateSupplierStats(); }
...
// ─── Inventory Load (with cache) ───
async function loadInventoryItems() {
    Perf.start('Inventory Load');
    const cached = cacheGet(INVENTORY_CACHE_KEY);
    if (cached) {
        allInventoryItems = cached;
        console.log(`✅ [PURCHASING] Inventory cache hit! ${cached.length} items`);
    ...
    try {
        ...
        console.log(`📦 [PURCHASING] Inventory fetched: ${allInventoryItems.length} items`);
...
async function refreshInventoryBackground() {
    try {
        ...
        snap.forEach(doc => allInventoryItems.push({ id: doc.id, ...doc.data() }));
        cacheSet(INVENTORY_CACHE_KEY, allInventoryItems);
        console.log('🔄 [PURCHASING] Inventory cache refreshed background');
    } catch(e) { console.warn('Background inventory refresh failed:', e); }
}

// ─── Suppliers Load (with cache) ───
async function loadSuppliersData() {
    Perf.start('Suppliers Load');
    const cached = cacheGet(SUPPLIERS_CACHE_KEY);
    if (cached) {
        allSuppliers = cached;
        console.log(`✅ [PURCHASING] Suppliers cache hit! ${cached.length} suppliers`);
        Perf.end('Suppliers Load');
        refreshSuppliersBackground();
        return;
    }
    try {
        ...
        console.log(`🏪 [PURCHASING] Suppliers fetched: ${allSuppliers.length}`);
    } catch(e) { console.error('Suppliers load error:', e); }
    Perf.end('Suppliers Load');
}

async function refreshSuppliersBackground() {
    try {
        const snap = await db.collection('suppliers').orderBy('name').get();
        allSuppliers = [];
        snap.forEach(doc => allSuppliers.push({ id: doc.id, ...doc.data() }));
        cacheSet(SUPPLIERS_CACHE_KEY, allSuppliers);
        console.log('🔄 [PURCHASING] Suppliers cache refreshed background');
    } catch(e) { console.warn('Background suppliers refresh failed:', e); }
}

// ─── Purchases Real-time (always fresh) ───
function loadPurchases() {
    ...
    db.collection('purchases').orderBy('createdAt', 'desc').onSnapshot((snap) => {
        ...
        console.log(`📄 [PURCHASING] Purchases updated: ${allPurchases.length}`);
        ...
        if (activeTab === 'tab-returns-btn')  renderReturnsDashboard();
        if (activeTab === 'tab-reports-btn')  loadPurchasingReports();

        if (selectedReturnBill) {
            const freshBill = allPurchases.find(p => p.id === selectedReturnBill.id);
            if (freshBill) selectedReturnBill = freshBill;
        }

        // ⚡ Reveal page after first data load
...
function updateBadges() {
    ...
    pBadge.textContent = pending; pBadge.style.display = pending > 0 ? 'inline-block' : 'none';
    const aBadge = document.getElementById('approvedBadge');
    aBadge.textContent = approved; aBadge.style.display = approved > 0 ? 'inline-block' : 'none';
...
function updateAlertBanner() {
    ...
    const pendingReturns = allPurchaseReturns.filter(r => r.status === 'pending_return').length;
    ...
    if (pending > 0 && isManagerUser()) {
        title.textContent    = `${pending} Pending Purchase Approval${pending > 1 ? 's' : ''}`;
        subtitle.textContent = 'Reports DB එකේ review කරන්න';
        banner.classList.add('show'); return;
    }
    if (pendingReturns > 0 && isManagerUser()) {
        title.textContent    = `${pendingReturns} Pending Return Approval${pendingReturns > 1 ? 's' : ''}`;
        subtitle.textContent = '🔄 Returns tab එකේ review කරන්න';
        banner.classList.add('show'); return;
    }
...
async function removeBillPhoto() {
    if (uploadedPhotoPath && uploadedPhotoUrl !== document.getElementById('billPhotoUrl').value.trim()) {
        try { await storage.ref(uploadedPhotoPath).delete(); } catch(e) { console.warn(e); }
    }
...
function renderSupplierSuggestions(suppliers, query = '') {
    ...
    suppliers.slice(0, 10).forEach(sup => {
        html += `<div class="supplier-suggestion-item" onclick="selectSupplier('${sup.id}')">
            ...
            ${sup.phone ? `<div class="sup-phone">📞 ${escapeHtml(sup.phone)}</div>` : ''}
        </div>`;
    });
    if (query && !suppliers.some(s => s.name.toLowerCase() === query)) {
        html += `<div class="supplier-suggestion-item add-new" onclick="useNewSupplier('${query.replace(/'/g, "\\'")}')">➕ Add NEW: "${escapeHtml(query)}"</div>`;
    }
...
function selectSupplier(supId) {
    const sup = allSuppliers.find(s => s.id === supId); if (!sup) return;
...
function onPaymentChange() {
    ...
    } else { dueDateWrap.style.display = 'none'; dueDateInput.value = ''; }
}
...
function removeItemRow(rowId) {
    ...
    if (document.getElementById('itemRowsList').children.length === 0) document.getElementById('noItemsMsg').style.display = 'block';
    calculateGrandTotal();
}
function searchItems(rowId) {
    ...
    filtered.forEach(item => {
        html += `<div class="item-suggestion-item" onclick="selectItem('${rowId}', '${item.id}')">
            <div class="it-name">📦 ${escapeHtml(item.itemName)}</div>
            <div class="it-info">${escapeHtml(item.category||'No category')} | Current: ${dispQty(item.currentStock)} ${escapeHtml(item.unit||'')} | Rs. ${(item.pricePerUnit||0).toFixed(2)}/${escapeHtml(item.unit||'')}</div>
        </div>`;
    });
    suggestions.innerHTML = html; suggestions.classList.add('show');
}
function selectItem(rowId, itemId) {
    const item = allInventoryItems.find(i => i.id === itemId); if (!item) return;
    const row  = document.getElementById(rowId);
    ...
    document.querySelectorAll('.item-row').forEach(row => {
        ...
        if (itemId && qty > 0 && total > 0) {
            ...
            if (item) items.push({
                ...
                unit: item.unit, quantity: fmtQty(qty), totalCost: fmtQty(total),
                pricePerUnit: fmtQty(total / qty), previousPrice: item.pricePerUnit || 0
            });
        }
    });
    ...
    if (!finalSupplierId) {
        ...
        if (existing) {
            ...
            if (supplierPhone && supplierPhone !== existing.phone) {
                await db.collection('suppliers').doc(existing.id).update({ phone: supplierPhone, updatedAt: firebase.firestore.FieldValue.serverTimestamp() });
            }
        } else {
            const newSup = await db.collection('suppliers').add({
                name: supplierName, phone: supplierPhone || '', email: '', address: '', notes: '',
                totalPurchases: 0, totalValue: 0, lastPurchaseDate: null, firstPurchaseDate: null,
                createdAt: firebase.firestore.FieldValue.serverTimestamp(), createdBy: currentUser.nickname
            });
        ...
        if (sup && supplierPhone !== sup.phone) {
            await db.collection('suppliers').doc(finalSupplierId).update({ phone: supplierPhone, updatedAt: firebase.firestore.FieldValue.serverTimestamp() });
        }
    }

    const purchaseData = {
        date, supplierId: finalSupplierId, supplierName, supplierPhone, billNumber,
        paymentMethod, dueDate: dueDate || null, billPhotoUrl: billPhotoUrl || '',
        billPhotoPath: uploadedPhotoPath || '', items, itemsCount: items.length,
        billTotal: fmtQty(billTotal), notes, status: 'pending_approval',
        paymentStatus: paymentMethod === 'cash' ? 'paid' : 'unpaid',
        paidDate: paymentMethod === 'cash' ? date : null, paidNotes: '',
        approvedBy: null, approvedByName: null, approvedAt: null, approvalNotes: '',
        rejectedReason: '', stockUpdated: false, returnStatus: 'none',
        returnedValue: 0, hasReturns: false, paymentProofUrl: '', paymentProofPath: '',
...
function autoResetForm() {
    ...
    itemRowCounter = 0; uploadedPhotoUrl = ''; uploadedPhotoPath = '';
    calculateGrandTotal();
}
...
function buildBillCard(p, statusClass, badgeText, messageType, messageContent, extraActions = '') {
    ...
    if (p.dueDate && p.paymentStatus === 'unpaid' && p.status === 'approved') {
        ...
        const isOverdue = p.dueDate < today; const isToday = p.dueDate === today;
    ...
    if (p.paymentStatus === 'paid' && p.status === 'approved') {
        dueWarning = `<div style="padding:10px 14px; background:rgba(76,175,80,0.1); border:1px solid rgba(76,175,80,0.3); border-radius:8px; margin-top:10px; font-size:13px; color:#4CAF50;">
            ✅ PAID${p.paidDate ? ` on ${p.paidDate}` : ''}${p.paidNotes ? `<br>📝 ${escapeHtml(p.paidNotes)}` : ''}
            ${p.paymentProofUrl ? `<br><button onclick="openPhotoFullscreen('${p.paymentProofUrl}')" style="background:rgba(33,150,243,0.15); color:#2196F3; border:none; padding:4px 12px; border-radius:6px; cursor:pointer; font-size:12px; margin-top:6px;">📸 View Payment Proof</button>` : ''}
        </div>`;
    }
    let returnChip = '';
    ...
    return `<div class="bill-card ${statusClass}">
        <div class="bc-header">
            <div class="bc-left">
                <span class="bc-bill-num">📄 ${escapeHtml(p.billNumber)}</span>
                <span class="bc-date">📅 ${escapeHtml(p.date)}</span>
        ...
        <div class="bc-supplier">🏪 ${escapeHtml(p.supplierName)}${p.supplierPhone?` <span style="color:#888; font-size:13px;">📞 ${escapeHtml(p.supplierPhone)}</span>`:''}</div>
        <div class="bc-stats">
        ...
        <div style="font-size:12px; color:#aaa; padding:8px 0; border-top:1px solid rgba(255,255,255,0.05); margin-top:8px;">
            <strong>📦 Items:</strong> ${escapeHtml(itemsList)}
        </div>
        ${p.notes ? `<div style="font-size:12px; color:#888; margin-top:4px;">📝 ${escapeHtml(p.notes)}</div>` : ''}
        ${returnChip}${dueWarning}
        ...
        <div class="bc-actions">
            ...
            ${photoBtn}${extraActions}
        </div>
        <div style="font-size:11px; color:#666; margin-top:6px;">👤 ${escapeHtml(p.performedByName||'-')} | 🕐 ${formatTimestamp(p.createdAt)}</div>
    </div>`;
}

function renderPendingBills() {
    const list = document.getElementById('pendingBillsList'); if (!list) return;
...
function renderApprovedBills() {
    const list = document.getElementById('approvedBillsList'); if (!list) return;
    ...
    let html = '';
    approved.forEach(p => {
        const msg = `<strong>✅ Approved by ${escapeHtml(p.approvedByName||'Manager')}</strong>${p.approvalNotes?`📝 ${escapeHtml(p.approvalNotes)}`:''}<br>📅 ${formatTimestamp(p.approvedAt)}${p.stockUpdated?'<br>✅ Inventory stock updated.':''}`;
        html += buildBillCard(p, 'approved', '✅ Approved', 'approve', msg);
    });
    list.innerHTML = html;
}
function renderRejectedBills() {
    const list = document.getElementById('rejectedBillsList'); if (!list) return;
    ...
    let html = '';
    rejected.forEach(p => {
        const msg     = `<strong>❌ Rejected by ${escapeHtml(p.approvedByName||'Manager')}</strong>📝 <strong>Reason:</strong> ${escapeHtml(p.rejectedReason||'No reason')}<br>🔄 Edit & resubmit කරන්න!`;
...
function updateCreditBadges() {
    const overdue  = getOverdueBills(); const today   = getDueTodayBills();
    const upcoming = getUpcomingBills(); const allUnpaid = getAllUnpaidBills(); const paid = getPaidBills();
    ...
    setBadge('ctOverdueBadge', overdue.length); setBadge('ctTodayBadge', today.length);
    setBadge('ctUpcomingBadge', upcoming.length); setBadge('ctAllUnpaidBadge', allUnpaid.length); setBadge('ctPaidBadge', paid.length);
}
...
function renderCreditTab() {
    ...
    bills.forEach(p => {
        ...
        if (p.dueDate) {
            if (p.dueDate < today) { const daysOverdue = Math.floor((new Date(today)-new Date(p.dueDate))/(1000*60*60*24)); dueLabel = `🚨 OVERDUE by ${daysOverdue} day${daysOverdue>1?'s':''}`; dueColor = '#ff4444'; }
            else if (p.dueDate === today) { dueLabel = '⏰ DUE TODAY'; dueColor = '#f0a500'; }
            else { const daysLeft = Math.floor((new Date(p.dueDate)-new Date(today))/(1000*60*60*24)); dueLabel = `⏳ ${daysLeft} day${daysLeft>1?'s':''} left`; dueColor = '#2196F3'; }
        }
        ...
        let amountDisplay = '';
        if (hasReturns && !isPaid) {
            amountDisplay = `<div style="background:rgba(0,0,0,0.3); border-radius:8px; padding:12px; margin-top:10px;">
                <div style="display:flex; justify-content:space-between; padding:4px 0; font-size:13px;"><span style="color:#aaa;">💰 Original Bill:</span><span>Rs. ${billTotal.toLocaleString('en-LK')}</span></div>
                <div style="display:flex; justify-content:space-between; padding:4px 0; font-size:13px;"><span style="color:#9C27B0;">🔄 Returned:</span><span style="color:#9C27B0; font-weight:600;">- Rs. ${returnedValue.toLocaleString('en-LK')}</span></div>
                <div style="display:flex; justify-content:space-between; padding:8px 0 4px; font-size:15px; border-top:1px solid rgba(255,255,255,0.1); margin-top:6px;"><span style="color:#4CAF50; font-weight:700;">💵 NET PAYABLE:</span><span style="color:#4CAF50; font-weight:700; font-size:17px;">Rs. ${netPayable.toLocaleString('en-LK')}</span></div>
            </div>`;
        }
        html += `<div class="bill-card ${isPaid?'approved':'rejected'}" style="border-left-color:${isPaid?'#4CAF50':(cardType==='overdue'?'#ff4444':cardType==='today'?'#f0a500':'#FF9800')};">
            <div class="bc-header">
                <div class="bc-left">
                    ...
                    <span class="bc-date">📅 ${escapeHtml(p.date)}</span>
                    <span class="bc-payment-badge ${p.paymentMethod}">${paymentIcons[p.paymentMethod]||p.paymentMethod}</span>
                    ${hasReturns?`<span style="background:rgba(156,39,176,0.15); color:#9C27B0; padding:3px 10px; border-radius:8px; font-size:11px; font-weight:700;">🔄 ${p.returnStatus==='fully_returned'?'FULLY':'PARTIALLY'} RETURNED</span>`:''}
                </div>
            ...
            <div class="bc-stats">
                <div class="bc-stat"><div class="bs-value">${p.itemsCount||0}</div><div class="bs-label">Items</div></div>
                <div class="bc-stat"><div class="bs-value" style="color:#FF9800;">Rs.${billTotal.toLocaleString('en-LK')}</div><div class="bs-label">Bill Total</div></div>
                ${p.dueDate?`<div class="bc-stat"><div class="bs-value" style="color:${dueColor};">${escapeHtml(p.dueDate)}</div><div class="bs-label">Due Date</div></div>`:''}
            </div>
            ${amountDisplay}
            ${isPaid&&p.paidDate?`<div style="padding:10px 14px; background:rgba(76,175,80,0.1); border:1px solid rgba(76,175,80,0.3); border-radius:8px; margin-top:10px; font-size:13px; color:#4CAF50;">
                ✅ PAID on ${escapeHtml(p.paidDate)} - Rs. ${(p.paidAmount||netPayable).toLocaleString('en-LK')}
                ${p.paidNotes?`<br>📝 ${escapeHtml(p.paidNotes)}`:''}
                ${p.paymentProofUrl?`<br><button onclick="openPhotoFullscreen('${p.paymentProofUrl}')" style="background:rgba(33,150,243,0.15); color:#2196F3; border:none; padding:4px 12px; border-radius:6px; cursor:pointer; font-size:12px; margin-top:6px;">📸 View Payment Proof</button>`:''}
            </div>`:''}
            <div class="bc-actions">
                ...
                ${!isPaid&&isManagerUser()?`<button class="bc-btn" style="background:#4CAF50; color:white;" onclick="openMarkPaidModal('${p.id}')">💰 Mark Paid (Rs. ${netPayable.toLocaleString('en-LK')})</button>`:''}
...
function openMarkPaidModal(billId) {
    ...
    let bodyHtml = `<div style="background:#0f3460; padding:14px; border-radius:8px;">
        <div style="font-size:14px; color:#FF9800; font-weight:700; margin-bottom:8px;">🏪 ${escapeHtml(p.supplierName)}</div>
        <div style="font-size:13px; color:#aaa;">📄 ${escapeHtml(p.billNumber)}</div>`;
    if (hasReturns) {
        bodyHtml += `<div style="margin-top:10px; padding-top:10px; border-top:1px solid rgba(255,255,255,0.1);">
            <div style="display:flex; justify-content:space-between; padding:3px 0; font-size:12px;"><span style="color:#aaa;">💰 Bill Total:</span><span>Rs. ${billTotal.toLocaleString('en-LK')}</span></div>
            <div style="display:flex; justify-content:space-between; padding:3px 0; font-size:12px;"><span style="color:#9C27B0;">🔄 Returned:</span><span style="color:#9C27B0;">- Rs. ${returnedValue.toLocaleString('en-LK')}</span></div>
            <div style="display:flex; justify-content:space-between; padding:6px 0; font-size:16px; border-top:1px solid rgba(255,255,255,0.1); margin-top:4px; font-weight:700;"><span style="color:#4CAF50;">💵 NET PAYABLE:</span><span style="color:#4CAF50;">Rs. ${netPayable.toLocaleString('en-LK')}</span></div>
        </div>`;
    } else {
...
async function confirmMarkPaid() {
    ...
    try {
        await db.collection('purchases').doc(markPaidBillId).update({
            paymentStatus: 'paid', paidDate, paidNotes, paidAmount: fmtQty(netPayable),
            paidBy: currentUser.nickname, paidByName: currentUser.name,
            paidAt: firebase.firestore.FieldValue.serverTimestamp(),
            paymentProofUrl: proofUrl || '', paymentProofPath: paymentProofPhotoPath || ''
        });
...
async function markUnpaid(billId) {
    ...
    try {
        await db.collection('purchases').doc(billId).update({
            paymentStatus: 'unpaid', paidDate: null, paidNotes: '', paidAmount: 0,
            paidBy: null, paidByName: null, paidAt: null, paymentProofUrl: '', paymentProofPath: ''
        });
...
function viewBillDetails(billId) {
    ...
    let itemsHtml = `<table style="width:100%; border-collapse:collapse; font-size:13px;">
        <thead><tr style="background:rgba(255,152,0,0.1);">
            <th style="padding:10px; text-align:left; color:#FF9800;">Item</th>
            <th style="padding:10px; color:#FF9800;">Qty</th>
            <th style="padding:10px; color:#FF9800;">Per Unit</th>
            <th style="padding:10px; color:#FF9800;">Total</th>
        </tr></thead><tbody>`;
    (p.items||[]).forEach(item => {
        itemsHtml += `<tr style="border-bottom:1px solid rgba(255,255,255,0.05);">
            <td style="padding:10px;"><strong>${escapeHtml(item.itemName)}</strong></td>
            <td style="padding:10px; text-align:center;">${dispQty(item.quantity)} ${escapeHtml(item.unit||'')}</td>
            <td style="padding:10px; text-align:center; color:#FF9800;">Rs. ${(item.pricePerUnit||0).toFixed(2)}</td>
            <td style="padding:10px; text-align:right; color:#FF9800; font-weight:700;">Rs. ${(item.totalCost||0).toLocaleString('en-LK')}</td>
        </tr>`;
    });
    itemsHtml += `<tr style="background:rgba(255,152,0,0.08); font-weight:700;">
        <td colspan="3" style="padding:12px; text-align:right; color:#FF9800;">GRAND TOTAL:</td>
        <td style="padding:12px; text-align:right; color:#FF9800; font-size:16px;">Rs. ${(p.billTotal||0).toLocaleString('en-LK')}</td>
    </tr></tbody></table>`;
    const paymentIcons = { cash: '💵 Cash', credit: '📋 Credit', bank: '🏦 Bank Transfer' };
    const billTotal = p.billTotal||0; const returnedValue = p.returnedValue||0; const netPayable = getNetPayable(p); const hasReturns = returnedValue > 0;
    let netSection = '';
    if (hasReturns) {
        netSection = `<div style="background:rgba(76,175,80,0.08); border:1px solid rgba(76,175,80,0.3); padding:14px; border-radius:8px; margin-bottom:14px;">
            <div style="font-size:14px; color:#4CAF50; font-weight:700; margin-bottom:10px;">💰 Payment Calculation:</div>
            <div style="display:flex; justify-content:space-between; padding:4px 0; font-size:13px;"><span style="color:#aaa;">Original Bill Total:</span><span>Rs. ${billTotal.toLocaleString('en-LK')}</span></div>
            <div style="display:flex; justify-content:space-between; padding:4px 0; font-size:13px;"><span style="color:#9C27B0;">🔄 Returned Value:</span><span style="color:#9C27B0;">- Rs. ${returnedValue.toLocaleString('en-LK')}</span></div>
            <div style="display:flex; justify-content:space-between; padding:8px 0 4px; font-size:16px; border-top:1px solid rgba(76,175,80,0.3); margin-top:6px; font-weight:700;"><span style="color:#4CAF50;">💵 NET PAYABLE:</span><span style="color:#4CAF50;">Rs. ${netPayable.toLocaleString('en-LK')}</span></div>
        </div>`;
    }
    let proofSection = '';
    if (p.paymentProofUrl) {
        proofSection = `<div style="margin-bottom:14px; text-align:center;">
            <div style="font-size:13px; color:#4CAF50; font-weight:600; margin-bottom:8px;">📸 Payment Proof:</div>
            <img src="${p.paymentProofUrl}" style="max-width:300px; max-height:300px; border-radius:8px; cursor:pointer; border:2px solid rgba(76,175,80,0.3);" onclick="openPhotoFullscreen('${p.paymentProofUrl}')">
        </div>`;
    }
    document.getElementById('billDetailBody').innerHTML = `
        <div style="background:#0f3460; padding:14px; border-radius:8px; margin-bottom:14px;">
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; font-size:13px;">
                <div><strong style="color:#FF9800;">📅 Date:</strong> ${escapeHtml(p.date||'-')}</div>
                <div><strong style="color:#FF9800;">🏪 Supplier:</strong> ${escapeHtml(p.supplierName||'-')}</div>
                <div><strong style="color:#FF9800;">📞 Phone:</strong> ${escapeHtml(p.supplierPhone||'N/A')}</div>
                <div><strong style="color:#FF9800;">📄 Bill #:</strong> ${escapeHtml(p.billNumber||'-')}</div>
                <div><strong style="color:#FF9800;">💳 Payment:</strong> ${paymentIcons[p.paymentMethod]||p.paymentMethod}</div>
                ${p.dueDate?`<div><strong style="color:#FF9800;">📅 Due:</strong> ${escapeHtml(p.dueDate)}</div>`:''}
                ${p.returnStatus&&p.returnStatus!=='none'?`<div><strong style="color:#9C27B0;">🔄 Return Status:</strong> ${escapeHtml(p.returnStatus)}</div>`:''}
                ${p.returnedValue?`<div><strong style="color:#9C27B0;">💰 Returned Value:</strong> ${fmt(p.returnedValue)}</div>`:''}
            </div>
        </div>
        ${netSection}
        ${p.billPhotoUrl?`<div style="margin-bottom:14px; text-align:center;">
            <div style="font-size:13px; color:#2196F3; font-weight:600; margin-bottom:8px;">📸 Bill Photo:</div>
            <img src="${p.billPhotoUrl}" style="max-width:300px; max-height:300px; border-radius:8px; cursor:pointer; border:2px solid rgba(255,152,0,0.3);" onclick="openPhotoFullscreen('${p.billPhotoUrl}')">
        </div>`:''}
        ${proofSection}
        <div style="background:#0f3460; padding:14px; border-radius:8px; overflow-x:auto;">${itemsHtml}</div>
        ${p.notes?`<div style="background:rgba(255,255,255,0.03); padding:12px; border-radius:8px; margin-top:12px; font-size:13px; color:#ccc;">📝 ${escapeHtml(p.notes)}</div>`:''}
        <div style="font-size:11px; color:#666; margin-top:14px;">
            👤 ${escapeHtml(p.performedByName||'-')} | 🕐 ${formatTimestamp(p.createdAt)}
            ${p.approvedAt?`<br>✅ ${p.status==='approved'?'Approved':'Rejected'}: ${escapeHtml(p.approvedByName||'-')} | 🕐 ${formatTimestamp(p.approvedAt)}`:''}
            ${p.paidAt?`<br>💰 Paid: ${escapeHtml(p.paidByName||'-')} | 🕐 ${formatTimestamp(p.paidAt)} | Amount: Rs. ${(p.paidAmount||0).toLocaleString('en-LK')}`:''}
        </div>`;
...
function updateReturnStats() {
    const total    = allPurchaseReturns.length;
    const pending  = allPurchaseReturns.filter(r => r.status === 'pending_return').length;
    const approved = allPurchaseReturns.filter(r => r.status === 'approved').length;
    const approvedValue = allPurchaseReturns.filter(r => r.status === 'approved').reduce((sum,r) => sum+(r.returnTotal||0), 0);
    const totalEl    = document.getElementById('ret-stat-total');
    const pendingEl  = document.getElementById('ret-stat-pending');
    const approvedEl = document.getElementById('ret-stat-approved');
    const valueEl    = document.getElementById('ret-stat-value');
    if (totalEl)    totalEl.textContent    = total;
    if (pendingEl)  pendingEl.textContent  = pending;
    if (approvedEl) approvedEl.textContent = approved;
    if (valueEl)    valueEl.textContent    = formatSupCurrency(approvedValue);
}
function updateReturnBadges() {
    const pending  = allPurchaseReturns.filter(r => r.status === 'pending_return').length;
    const mainBadge = document.getElementById('returnsBadge');
    if (mainBadge) { mainBadge.textContent = pending; mainBadge.style.display = pending > 0 ? 'inline-block' : 'none'; }
    const subBadge = document.getElementById('retPendingBadge');
    if (subBadge)  { subBadge.textContent  = pending; subBadge.style.display  = pending > 0 ? 'inline-block' : 'none'; }
}
...
function showReturnSubTab(name, btnEl = null) {
    currentReturnSubTab = name;
    document.querySelectorAll('#section-returns .nav-tabs .nav-tab').forEach(t => t.classList.remove('active'));
    if (btnEl) btnEl.classList.add('active');
    else { const autoBtnMap = { 'new-return':'ret-new-btn','pending-returns':'ret-pending-btn','approved-returns':'ret-approved-btn','rejected-returns':'ret-rejected-btn','all-returns':'ret-all-btn' }; const autoBtn = document.getElementById(autoBtnMap[name]); if (autoBtn) autoBtn.classList.add('active'); }
    document.querySelectorAll('.return-sub-section').forEach(sec => { sec.style.display = 'none'; });
    const target = document.getElementById('sub-' + name); if (target) target.style.display = 'block';
    if (name==='pending-returns')  renderPendingReturns();
    if (name==='approved-returns') renderApprovedReturns();
    if (name==='rejected-returns') renderRejectedReturns();
    if (name==='all-returns')      renderAllReturns();
}
...
function selectReturnBill(billId) { const bill=allPurchases.find(p=>p.id===billId);if(!bill)return;selectedReturnBill=bill;document.getElementById('sbpTitle').textContent=`📄 Selected Bill: ${bill.billNumber}`;document.getElementById('sbpInfo').innerHTML=`<div class="sbp-info-item"><div class="sbp-info-label">Supplier</div><div class="sbp-info-value">🏪 ${escapeHtml(bill.supplierName)}</div></div><div class="sbp-info-item"><div class="sbp-info-label">Purchase Date</div><div class="sbp-info-value">📅 ${escapeHtml(bill.date)}</div></div><div class="sbp-info-item"><div class="sbp-info-label">Bill Total</div><div class="sbp-info-value">💰 ${fmt(bill.billTotal||0)}</div></div><div class="sbp-info-item"><div class="sbp-info-label">Items</div><div class="sbp-info-value">📦 ${bill.itemsCount||0}</div></div>`;document.getElementById('selectedBillPreview').classList.add('show');document.getElementById('returnStep2').style.display='block';renderReturnItemsForSelectedBill();document.getElementById('returnStep2').scrollIntoView({behavior:'smooth',block:'start'});searchBillsForReturn();}
...
async function saveSupplier() { const editId=document.getElementById('sup-edit-id').value.trim();const name=document.getElementById('sup-name').value.trim();const phone=document.getElementById('sup-phone').value.trim();const email=document.getElementById('sup-email').value.trim();const address=document.getElementById('sup-address').value.trim();const notes=document.getElementById('sup-notes').value.trim();if(!name){alert('⚠️ Name required!');return;}try{if(editId){await db.collection('suppliers').doc(editId).update({name,phone,email,address,notes,updatedAt:firebase.firestore.FieldValue.serverTimestamp()});}else{await db.collection('suppliers').add({name,phone,email,address,notes,totalPurchases:0,totalValue:0,lastPurchaseDate:null,firstPurchaseDate:null,createdAt:firebase.firestore.FieldValue.serverTimestamp(),createdBy:currentUser.nickname});}closeSupModal('sup-add-modal');cacheRemove(SUPPLIERS_CACHE_KEY);await loadSuppliersData();renderSuppliersList(allSuppliers);updateSupplierStats();}catch(e){alert('❌ '+e.message);}}
