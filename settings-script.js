// ============================================================
// BUONO - Settings Script v3.0
// Reuses Call Center styling pattern
// ============================================================
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const user = initPage('settingsDB', ['Admin', 'Manager']);
    if (!user) return;

    console.log('✅ User verified:', user.name, '|', user.access);
    loadSettings();
});
...
function populateForm(data) {
    ...
    if (installEl && data.defaultInstallments) installEl.value = data.defaultInstallments;
    ...
    if (templateArea) templateArea.addEventListener('input', updatePreview);
...
function updateStats() {
    ...
    if (businessEl) {
        const name = getVal('businessName') || 'Not Set';
        businessEl.textContent = name.length > 12 ? name.substring(0, 12) + '...' : name;
    }
    if (batchEl) batchEl.textContent = getVal('currentBatch') || 'W1';
    if (discountEl) discountEl.textContent = discountCodes.length;

    if (paymentEl) {
        let activeMethods = 0;
        ['paymentCash', 'paymentBank', 'paymentCard', 'paymentOnline'].forEach(id => {
            if (getCheck(id)) activeMethods++;
        });
        paymentEl.textContent = activeMethods + '/4';
    }

    if (updatedEl && currentSettings.updatedAt) {
        try {
            const date = currentSettings.updatedAt.toDate();
...
function renderDiscountCodes() {
    ...
    if (discountCodes.length === 0) {
        list.innerHTML = '<div class="no-lead-msg"><div class="no-lead-icon">🏷️</div>No discount codes yet</div>';
        return;
    }
    ...
    list.innerHTML = discountCodes.map((code, index) => `
        <div class="discount-item">
            <div class="discount-info">
                <span class="discount-code-badge">${code.code}</span>
                <span class="discount-value-text">
                    ${code.type === 'percentage' ? code.value + '% OFF' : 'Rs. ' + code.value + ' OFF'}
                </span>
                <span class="discount-type-badge">${code.type}</span>
            </div>
            <button class="btn-icon delete" onclick="removeDiscountCode(${index})" title="Remove">🗑️</button>
        </div>
    `).join('');
}
...
function addDiscountCode() {
    ...
    if (discountCodes.find(d => d.code === code)) { showToast('Code already exists!', 'warning'); return; }
